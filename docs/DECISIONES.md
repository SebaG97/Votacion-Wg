...
## DEC-025 - Cierre De Gaps De Backend Para El Panel Administrativo (Mision 10)

Fecha: 2026-09-01

Contexto: Al revisar que endpoints existen realmente para construir el panel administrativo de la Mision 10 aparecieron dos huecos que, a diferencia de DEC-020/DEC-023 (control de acceso pendiente a proposito), no estaban documentados como decision deliberada: eran un olvido. Primero, `POST /api/v1/padron/importaciones` (Mision 04) no tenia `require_admin` a pesar de ser mas sensible que crear/abrir/cerrar una votacion (protegidos desde DEC-021): puede reimportar/recrear todo el padron, personas, matrimonios, unidades electorales e incidencias. Segundo, no existia ningun `GET /api/v1/votaciones` que listara todas las votaciones: sin el, el panel no tenia forma de descubrir que `votacion_id` administrar salvo conocerlo de memoria o via `GET /votaciones/abierta` (DEC-023), que solo sirve mientras hay una ABIERTA. Ademas, `IncidenciaPadron.resuelto_por`/`resuelto_at` existen en el modelo desde la Mision 03 pero ningun endpoint los escribia nunca: el panel necesita poder marcar una incidencia como revisada.

Decision: Se agregan cuatro endpoints, los cuatro protegidos por `require_admin` (DEC-021):

1. `require_admin` se agrega a `POST /api/v1/padron/importaciones` (`backend/app/api/v1/endpoints/padron.py`, ahora `router = APIRouter(dependencies=[Depends(require_admin)])`, mismo patron que `votaciones.py`).
2. `GET /api/v1/votaciones`: lista todas las votaciones (`id`, `nombre`, `estado`, fechas), mas nueva primero. Reusa `VotacionResponse` (ya tenia exactamente estos campos) en vez de un schema nuevo; el servicio nuevo es `listar_votaciones` (`app/services/votacion.py`).
3. `GET /api/v1/padron/importaciones`: historial de importaciones, mas nueva primero. Reusa `ImportacionPadronResponse`, que ya existia desde la Mision 04 sin ningun endpoint de lectura.
4. `GET /api/v1/padron/incidencias` (filtrable por `severidad`, `tipo` y `resuelta`, los tres opcionales) y `POST /api/v1/padron/incidencias/{id}/resolver` (body `{usuario}`, setea `resuelto_por`/`resuelto_at`, 404 si no existe, 409 si ya estaba resuelta). Ambos nuevos en `app/services/padron/administracion.py`, con el schema nuevo `IncidenciaPadronResponse`.

El punto 4 es deliberadamente limitado: `resolver_incidencia` es trazabilidad administrativa pura ("ya lo revise"), no un cambio de habilitacion. **No** recalcula `UnidadElectoral.estado` de ninguna forma. Hacerlo automaticamente exigiria resolver antes DEC-012 (bajas de personas), DEC-013 (circulos de postulantes) y DEC-014 (doble rol de jefes consagrados), las tres todavia pendientes de negocio; mezclar esa decision futura con esta trazabilidad seria adelantarse a algo que el negocio no definio. Si mas adelante el negocio resuelve esas tres decisiones, aplicarlas es un cambio acotado en `app/services/padron/administracion.py` o en un endpoint nuevo de habilitacion manual, no algo que "resolver" deba hacer por su cuenta.

Consecuencias: `backend/tests/test_padron_endpoint.py` gana `test_post_importaciones_padron_sin_token_da_401_o_403` y actualiza sus tres pruebas existentes para mandar el header administrativo. `backend/tests/test_padron_administracion_endpoint.py` (nuevo) cubre listado y filtros de incidencias, resolver (incluida la verificacion explicita de que `UnidadElectoral.estado` no cambia y que resolver dos veces da 409) y el control de acceso de los tres endpoints de padron nuevos. `backend/tests/test_votacion_endpoint.py` gana la prueba de `GET /votaciones` y su control de acceso. Ningun modelo ni migracion nueva hace falta: las cuatro columnas que se empiezan a leer/escribir (`resuelto_por`, `resuelto_at`) o a exponer por primera vez (el resto de `ImportacionPadron`/`Votacion`) ya existian.

## DEC-026 - Doble Rol De Jefes Consagrados (DEC-014): Resuelta Sin Cambio De Codigo

Fecha: 2026-09-01

Contexto: DEC-014 quedo pendiente de negocio desde la Mision 02. La Mision 06 (DEC-020) ya habia implementado el desenlace mas simple -- la persona con doble rol emite dos votos independientes, uno por cada unidad, sin restriccion cruzada -- explicitamente para no adelantarse a una decision que el negocio todavia no habia tomado. Al preparar la Mision 11 se releyo `docs/REGLAS_NEGOCIO.md`, seccion "Habilitacion Por Celular", que dice textualmente: "Persona cumple ambos roles: el sistema debe presentar opciones separadas y registrar cada voto con su unidad electoral." Esa linea estuvo en el requerimiento original desde la Mision 00 y responde exactamente la pregunta de DEC-014; nadie la habia cruzado con la decision pendiente hasta ahora.

Decision: DEC-014 queda resuelta -- confirmando el comportamiento ya implementado desde la Mision 06, no cambiandolo -- citando `REGLAS_NEGOCIO.md` como la fuente que ya lo definia. No se toca ningun codigo: `unidades_candidatas` (`backend/app/services/habilitacion.py`) y `registrar_voto` (`backend/app/services/voto.py`) ya separan las dos unidades del doble rol sin cruzar ninguna validacion entre ellas, y ya existe cobertura de prueba (`test_jefe_consagrado_con_doble_rol_permite_dos_votos_independientes` en `test_voto.py`, los casos de doble rol en `test_habilitacion.py`, y en el frontend -- Mision 09 -- los dos botones separados y etiquetados de `ResultadoConsulta.tsx`).

A diferencia de DEC-012 y DEC-013 (DEC-027, DEC-028 mas abajo), esta no es una resolucion del orquestador ante la falta de respuesta del negocio: es la lectura -- recien hecha -- de un requerimiento que ya estaba escrito desde el principio del proyecto.

Consecuencias: Ninguna migracion, endpoint ni prueba nueva hace falta. `docs/MISIONES.md` marca DEC-014 como resuelta en el resumen de la Mision 11.

## DEC-027 - Bajas De Personas (DEC-012): Resuelta Por El Orquestador -- Las 8 Unidades Sin Ningun Integrante Activo No Votan En Esta Eleccion

Fecha: 2026-09-01

Contexto: DEC-012 quedo pendiente de negocio desde la Mision 02: 118 personas con la marca `No ML` y 33 con observacion textual de baja, sin que el negocio hubiera definido si esas personas -y las unidades electorales que dependen de ellas- quedan excluidas del padron votante. DEC-016 (Mision 04) ya modelo el desenlace de "todos los integrantes de baja" como el estado `PENDIENTE_DEFINICION_BAJA`, evaluado con esta regla exacta: *"todas las personas que componen la unidad ... estan en estado `BAJA_NO_ML` o `BAJA_OBSERVACION`. Si al menos una persona sigue `ACTIVA`, la unidad no se marca pendiente por este motivo"* (`docs/DECISIONES.md`, DEC-016, regla 3). Sobre el Excel real, importado de nuevo para esta mision, esa regla produce exactamente 8 unidades electorales en `PENDIENTE_DEFINICION_BAJA` de las 314 totales -- por definicion de la regla misma, las 8 son unidades donde ningun integrante esta `ACTIVA`.

**Esta decision es una resolucion del orquestador, no una confirmacion del dueño real del padron.** Sebad autorizo explicitamente resolver DEC-012, DEC-013 y DEC-014 con el razonamiento mas logico disponible, cruzando `PADRON_ANALISIS.md`, `REGLAS_NEGOCIO.md` y los numeros reales ya importados -- sin consultarlo primero con quien administra el Excel. Esto es distinto de DEC-017, donde el dueño del padron confirmo por escrito, revisando el resultado de una mision ya corrida, cual era el comportamiento correcto. Si el dueño del padron da una respuesta distinta mas adelante, corregirla es un cambio acotado (el mismo tipo de cambio que ya se hizo para DEC-017): un `UPDATE` sobre el estado de esas 8 unidades y sobre el texto del mensaje que las describe, no una reimportacion ni una migracion.

El caso contrario -- una unidad con al menos un integrante activo -- ya lo resuelve el sistema hoy sin intervencion adicional: esa unidad ni siquiera entra en `PENDIENTE_DEFINICION_BAJA` (sigue habilitada o cae en otra regla de precedencia de DEC-016), el mismo principio que DEC-011 ya aplico para los viudos consagrados. La pregunta que queda, entonces, es exclusivamente sobre las 8 unidades donde absolutamente nadie sigue activo: no existe ninguna persona viva/activa que pueda presentarse con un celular a consultar esa unidad.

Decision: Para esta eleccion, las 8 unidades electorales sin ningun integrante activo quedan bloqueadas de forma **permanente**, no como "pendiente de definir". Se reutiliza el estado `PENDIENTE_DEFINICION_BAJA` que ya existe -- ya bloquea exactamente estas 8 unidades, sin necesitar ningun cambio de esquema ni de logica del importador -- pero deja de leerse como un estado transitorio a la espera de una respuesta y pasa a ser una decision tomada. El unico cambio de codigo asociado es de redaccion, no de logica: el mensaje que el frontend le muestra al usuario para este estado (y para `PENDIENTE_DEFINICION_POSTULANTES`, DEC-028) deja de decir "todavia" -- ver DEC-029... (continua en la decision de mensaje, ver mas abajo en esta misma entrada combinada con DEC-028).

Consecuencias: De las 314 unidades electorales, 265 siguen `HABILITADA` y 16 `BLOQUEADA_POR_INCIDENCIA` (sin cambio). Las 8 unidades de `PENDIENTE_DEFINICION_BAJA` no suman votos habilitables en esta eleccion -- el total votable de 265 no cambia respecto de lo que ya se contaba como habilitado hoy. `frontend/src/lib/motivos.ts` (`traducirMotivoNoDisponible`) cambia el mensaje de `PENDIENTE_DEFINICION_POSTULANTES`/`PENDIENTE_DEFINICION_BAJA` de "Todavía no está habilitada para votar." a "Esta unidad no tiene voto habilitado en esta elección.", con su prueba actualizada en `motivos.test.ts`. No hace falta ningun cambio en `frontend-admin`: no existe un mensaje equivalente orientado al votante ahi, solo las etiquetas administrativas ("Pendiente baja", "Pendiente postulantes") de `VotacionDetailPage.tsx`, que son nombres de estado para el operador, no una promesa de habilitacion futura, y no dependen de la palabra "todavia".

## DEC-028 - Circulos De Postulantes (DEC-013): Resuelta Por El Orquestador -- Las 25 Unidades No Votan Este Ciclo

Fecha: 2026-09-01

Contexto: DEC-013 quedo pendiente de negocio desde la Mision 02: circulos de postulantes (matrimonios no consagrados en formacion) sin que el negocio hubiera confirmado si corresponde un voto de bloque por cada uno o si no votan en esta eleccion. DEC-016 (Mision 04) modelo el estado `PENDIENTE_DEFINICION_POSTULANTES`, que aplica *"solo ... a `BLOQUE_NO_CONSAGRADO` cuyo circulo contiene la palabra 'POSTULANTE' en su nombre"*. Sobre el Excel real, importado de nuevo para esta mision, esa regla produce exactamente 25 unidades electorales en ese estado, de 314 totales.

Igual que DEC-027, **esta es una resolucion del orquestador bajo autorizacion explicita de Sebad, no una confirmacion del dueño real del Excel** -- a diferencia de DEC-017. Si el dueño del padron confirma o corrige esta resolucion mas adelante, aplicar su respuesta es un cambio acotado: un `UPDATE` sobre el estado de estas 25 unidades y sobre el mensaje que las describe.

El razonamiento: "postulante" en este padron implica matrimonios no consagrados todavia en formacion, sin el standing completo de miembro pleno del grupo. El paralelo mas cercano en cualquier organizacion con un periodo de prueba formal es que quienes siguen en formacion no votan en elecciones de gobierno interno hasta completar el proceso -- son parte del grupo, pero todavia no tienen el mismo derecho de decision que un miembro consagrado. No hay en `PADRON_ANALISIS.md` ni en `REGLAS_NEGOCIO.md` ningun elemento que sugiera lo contrario (por ejemplo, que un circulo de postulantes ya tenga jefe habilitado a votar por el bloque como si fuera un circulo consagrado pleno).

Decision: Para esta eleccion, las 25 unidades `BLOQUE_NO_CONSAGRADO` de circulos de postulantes quedan bloqueadas de forma **permanente**, no como "pendiente de definir" -- misma resolucion que DEC-027, mismo motivo de fondo (no hay standing pleno de miembro para ejercer ese voto en este ciclo). Se reutiliza `PENDIENTE_DEFINICION_POSTULANTES` sin cambio de esquema ni de logica del importador.

Consecuencias: De las 314 unidades electorales, 265 siguen `HABILITADA` y 16 `BLOQUEADA_POR_INCIDENCIA` (sin cambio). Sumando las 25 de esta decision y las 8 de DEC-027, 33 unidades quedan bloqueadas de forma permanente para esta eleccion; el total votable sigue siendo 265, igual que lo que ya se contaba como habilitado hoy. El cambio de mensaje del frontend (`frontend/src/lib/motivos.ts`, ver DEC-027) cubre a este estado tambien, ya que ambos comparten el mismo texto traducido.

## DEC-029 - Rate Limiting Por IP Sobre Los Endpoints Operativos Sin Control De Acceso

Fecha: 2026-09-01

Contexto: `POST /api/v1/habilitaciones/consultar` y `POST /api/v1/votaciones/{id}/votos` siguen sin ningun control de acceso (DEC-020), a proposito: son endpoints operativos que usa cualquier votante desde su celular, no el panel administrativo. Restringir por red (IP fija, VPN) no aplica a este despliegue: la votacion no ocurre en un evento de un dia con una red controlada, sino que el sistema queda expuesto en internet por dias o semanas. Agregar una contrasena compartida tampoco resuelve nada real -- no identifica a la persona que la usa, solo agrega friccion de UX sin aportar trazabilidad (la trazabilidad real, `emitido_por_persona_id`, ya existe desde la Mision 06 y no depende de esto). Una autenticacion real por votante (por ejemplo, verificacion por SMS contra el celular del padron) resolveria el problema de fondo, pero es un proyecto bastante mas grande, fuera de alcance de esta mision -- igual que ya quedo documentado como pendiente en DEC-020.

Decision: Se agrega rate limiting por IP a esos dos endpoints (unicamente esos dos; el resto ya esta protegido por `ADMIN_API_KEY`, DEC-021), con `slowapi` (`backend/app/core/rate_limit.py`): limite configurable via `RATE_LIMIT_POR_MINUTO` (`Settings`, default 20 requests/minuto), lo bastante generoso para que un votante real haciendo una consulta ocasional -incluida una recarga de pagina o un reintento tras un error de red- nunca lo note, pero bajo para elevar sustancialmente el costo de escanear el padron completo (1113 celulares) o de automatizar intentos de voto contra una unidad. Al superar el limite, responde `429 Too Many Requests` (manejador default de `slowapi`, sin cuerpo custom).

La IP se resuelve con `get_client_ip` (no el `get_remote_address` default de `slowapi`, que lee `request.client.host`): en un despliegue detras de un proxy o load balancer -DigitalOcean App Platform enruta siempre asi- `request.client.host` es la IP del proxy, no la del votante, lo que rate-limitaria a todos los votantes juntos como si fueran una sola persona. `get_client_ip` usa el primer valor de `X-Forwarded-For` si el header esta presente, y cae a `get_remote_address` si no. El header es falseable por quien le pega directo al backend sin pasar por el proxy, pero en este despliegue nadie deberia poder hacer eso (el backend no esta pensado para exponerse fuera del proxy de DigitalOcean), asi que no debilita la proteccion real; si el despliegue cambia y el backend queda accesible directamente, esta suposicion hay que revisarla.

Esto es explicitamente **no** un reemplazo de autenticacion real por votante: no identifica quien hace la request, solo limita cuantas puede hacer una misma IP en una ventana de tiempo. Sigue siendo posible que una persona autorizada consulte o intente votar por una unidad ajena si conoce el celular -- ese problema de fondo lo resuelve la verificacion `emitido_por_persona_id` de `registrar_voto` (Mision 06) para el voto en si, no la consulta de habilitacion, y ninguno de los dos verifica identidad real del dispositivo.

## DEC-030 - Login Administrativo Usuario/Contrasena Delante De `ADMIN_API_KEY`, Sin Tocar `require_admin`

Fecha: 2026-09-02

Contexto: Desde la Mision 10, `frontend-admin/src/routes/LoginPage.tsx` pedia pegar el `ADMIN_API_KEY` crudo (un token largo generado a mano) en un campo de contraseña. Sebad necesita entregarle acceso al panel a quien vaya a administrar la votacion sin exponerle ese token tecnico -- quiere un usuario y una contraseña convencionales (`admin` / `votacion2026`) para distribuir.

Decision: Se agrega `POST /api/v1/auth/login` (`backend/app/api/v1/endpoints/auth.py`), que recibe `{usuario, contrasena}` y los valida contra dos variables de entorno nuevas, `ADMIN_USERNAME` y `ADMIN_PASSWORD` (`Settings`, `backend/app/core/config.py` y `.env.example`) -- un unico par de credenciales fijo, no una tabla de usuarios ni un sistema de sesiones. Si `ADMIN_USERNAME`, `ADMIN_PASSWORD` **o** `ADMIN_API_KEY` no estan configurados en el servidor, el login responde `403` -- mismo criterio de "falla cerrado" que ya usa `require_admin` (DEC-021) con `ADMIN_API_KEY` solo: no tiene sentido dejar pasar un login cuyo token de salida no existe. Con credenciales incorrectas responde `401`. Con credenciales correctas, devuelve `{token: ADMIN_API_KEY}` -- el mismo valor que `require_admin` (`app/api/deps.py`) ya exige en el header `X-Admin-Token` de cada endpoint administrativo. Deliberadamente **no** se toca `require_admin` ni ningun endpoint ya protegido por el: el usuario/contraseña es una puerta de entrada nueva delante de la proteccion que ya existe, no un reemplazo de la arquitectura de autorizacion. `frontend-admin` sigue guardando ese token exactamente igual que antes (`adminToken.ts`, `sessionStorage`) y mandandolo como `X-Admin-Token` en cada request -- el resto del panel no nota el cambio.

Como la contraseña ahora es fija y corta (a diferencia del token largo que reemplaza) y el panel queda expuesto en internet igual que el resto del sistema (DEC-029), `POST /auth/login` reusa la infraestructura de `slowapi` de la Mision 11 (`app/core/rate_limit.py`) con un limite nuevo y separado, `RATE_LIMIT_LOGIN = "5/minute"` -- fijo, no configurable via `.env` como `RATE_LIMIT_POR_MINUTO`, porque este endpoint no lo usa un votante real de forma repetida, solo quien administra el panel, y cinco intentos por minuto ya hacen inviable adivinar una contraseña corta por fuerza bruta sin friccionar a un administrador real que se equivoca tipeando.

`frontend-admin/src/routes/LoginPage.tsx` cambia el campo unico de token por dos campos (usuario/contraseña) y llama a `POST /auth/login` en vez de pegar el token de forma optimista y validarlo contra `GET /votaciones` como hacia antes (Mision 10): ahora el servidor confirma las credenciales antes de que el frontend guarde nada, asi que no hace falta esa verificacion de segundo paso.

Consecuencias: `backend/tests/test_auth_endpoint.py` (nuevo) cubre credenciales correctas (devuelve el `ADMIN_API_KEY` exacto, y ese token efectivamente pasa `require_admin`), usuario incorrecto (401), contraseña incorrecta (401), variables de entorno faltantes -- las tres vacias y el caso parcial de solo faltar `ADMIN_API_KEY` -- (403), y el limite de intentos (429 al superar `RATE_LIMIT_LOGIN`). `frontend-admin/src/test/login.test.tsx` (nuevo) cubre el camino feliz y los tres errores (401/403/429) traducidos a mensajes en español. Ningun modelo ni migracion nueva hace falta.

## DEC-031 - El Visor De Padron No Incluye Ni Permite Cruzar `Voto`

Fecha: 2026-09-02

Contexto: Sebad pidio una vista del padron (personas, matrimonios, grupos y unidades electorales) navegable en tabla y filtrable, para poder consultar esos datos el dia de la votacion si hace falta -- circulo, estado de la persona, tipo y estado de la unidad electoral, nombre, documento, celular. El requerimiento fue explicito en excluir cualquier cruce contra la tabla `Voto` (que opcion eligio cada unidad electoral).

Decision: Se agrega `GET /api/v1/padron/personas` (`backend/app/api/v1/endpoints/padron.py`, protegido por `require_admin` como el resto del router) con la funcion de listado `listar_padron` (`app/services/padron/administracion.py`, junto a `listar_importaciones`/`listar_incidencias`), filtrable por `circulo`, `grupo_id`, `estado_persona`, `estado_unidad_electoral`, `tipo_unidad_electoral`, `nombre`, `documento` y `celular` (todos opcionales y combinables), y paginado (`pagina`/`tamanio_pagina`, maximo 200 por pagina). Cada fila es una `Persona` con su circulo (`Grupo.nombre`, unido por `Persona.grupo_id`), los datos de su `Matrimonio` (si tiene) y las unidades electorales que le corresponden -- puede haber dos si la persona tiene doble rol de jefe consagrado (DEC-020/DEC-026), resueltas con dos `outerjoin` separados (uno por `MATRIMONIO_CONSAGRADO` via `Persona.matrimonio_id`, otro por `BLOQUE_NO_CONSAGRADO` via `Persona.grupo_id` cuando `es_jefe_grupo`) que por el UNIQUE `(tipo, referencia_id)` de `UnidadElectoral` nunca multiplican filas.

La exclusion de `Voto` es deliberada, no un olvido: el sistema ya registra trazabilidad de quien voto a nivel de auditoria (`emitido_por_persona_id`, Mision 06, DEC-020), pero los resultados publicos evitan a proposito cruzar circulo por opcion porque muchos circulos tienen una sola unidad electoral, y ese cruce revelaria el voto individual de esa unidad (DEC-022, punto 2). Un visor de padron que permitiera filtrar o mostrar "que voto la unidad X" reabriria exactamente ese problema por una puerta lateral. `listar_padron` no importa el modelo `Voto` ni ninguna tabla relacionada, y ningun filtro ni campo de la respuesta se deriva de el -- este visor es para consultar "quien es quien, en que circulo, si esta habilitado", no "que opcion eligio".

`frontend-admin/src/routes/PadronPage.tsx` (nuevo) sigue el mismo patron que `IncidenciasPage.tsx` (Mision 10): filtros que arman la query string, tabla, y ahora ademas paginacion (50 filas por pagina, con controles Anterior/Siguiente) porque el padron tiene mas de mil personas. Un banner visible aclara que el visor no muestra ni permite cruzar que opcion voto cada unidad, mismo espiritu que el banner de `IncidenciasPage` sobre "revisar" una incidencia.

Consecuencias: `backend/tests/test_padron_administracion_endpoint.py` gana un bloque de pruebas para `GET /padron/personas`: listado completo, cada filtro por separado (circulo, nombre, documento, celular, estado de persona, estado y tipo de unidad electoral) contra el fixture sintetico de `test_importador_padron.py`, una combinacion de filtros, paginacion (tamaño de pagina chico para forzar varias paginas, verificando que no se repiten ids entre paginas) y que el endpoint sigue exigiendo `require_admin`. `frontend-admin/src/test/padron.test.tsx` (nuevo) cubre la carga inicial, que un filtro arma la query string y reinicia a la pagina 1, que la paginacion pide la pagina siguiente, y verifica explicitamente que ninguna request ni el DOM renderizado mencionan datos de voto. Ningun modelo ni migracion nueva hace falta: `listar_padron` solo lee columnas que ya existian.

## DEC-032 - Un Solo Punto De Acceso: Link Al Panel Administrativo Desde El Footer De `frontend/`, Sin Fusionar Los Dos Proyectos

Fecha: 2026-09-02

Contexto: `frontend/` (votante) y `frontend-admin/` (panel administrativo) son dos proyectos Vite completamente separados -- distinto `package.json`, build, tests y puerto de desarrollo (5175 y 5174 respectivamente). Sebad pidio que la pagina principal sea la de votacion y que desde ahi haya un link, abajo a la izquierda en un footer, hacia el panel administrativo, para no tener que conocer de memoria una segunda URL.

Decision: Se evaluaron tres opciones: (a) fusionar `frontend-admin/` dentro de `frontend/` como una ruta mas (ej. `/admin`), un solo proyecto y un solo puerto; (b) mantener los dos proyectos separados pero servirlos bajo el mismo dominio en produccion via ruteo por path (por ejemplo, dos componentes de una misma app de DigitalOcean App Platform, con rutas HTTP `/` y `/admin`); (c) mantener todo como esta y agregar solamente un link de un proyecto al otro. Sebad eligio la opcion (c) para este cambio: es la de menor riesgo -- no toca ningun router, contexto de autenticacion, config de build ni test existente de ninguno de los dos proyectos -- y no fuerza ninguna decision de infraestructura todavia no tomada.

Se agrega `frontend/src/components/SiteFooter.tsx`, montado una sola vez en `App.tsx` (junto a `<Routes>`, no dentro de cada pagina) para que aparezca en todas las rutas de `frontend/` sin duplicar el markup. Es un `<footer>` con `position: fixed` (`.site-footer` en `styles.css`) anclado abajo a la izquierda del viewport, respetando `env(safe-area-inset-*)` igual que ya hace `.app-shell` -- deliberadamente no se toco el layout de `.app-shell` (por ejemplo, pasarlo a `display: flex; flex-direction: column` para empujar un footer real al fondo del documento) para no arriesgar una regresion visual en la pantalla de votacion sin poder verificarla con un navegador real en esta sesion.

La URL del link es configurable via `VITE_ADMIN_PANEL_URL` (`frontend/.env.example`). Sin configurar, el valor por defecto depende de `import.meta.env.DEV`: en desarrollo (`vite dev`) apunta a `http://localhost:5174` (el puerto por defecto de `frontend-admin/`, hardcodeado a proposito para que el link funcione sin configuracion extra en un `clone` nuevo del repo); en un build de produccion cae a la ruta relativa `/admin`, pensada para la opcion (b) de arriba si en el futuro se configura ese ruteo por path en el mismo dominio -- sin esa configuracion de infraestructura (todavia no existe en este repo), el link de produccion no resuelve a nada y hay que setear `VITE_ADMIN_PANEL_URL` a la URL real de `frontend-admin/` desplegado. El chequeo de "variable sin configurar" usa `?.trim() || fallback` (no `??`) a proposito: `.env.example` documenta la variable con valor vacio, y `??` no cae al fallback ante un string vacio, solo ante `null`/`undefined` -- con `??` una copia literal de `.env.example` a `.env` habria roto el link en silencio.

Consecuencias: Ningun cambio en `frontend-admin/` ni en el backend. `frontend/src/components/SiteFooter.test.tsx` (nuevo) verifica que el link se renderiza con el fallback de desarrollo. `frontend/.env.example` documenta `VITE_ADMIN_PANEL_URL` y su comportamiento por defecto. Pendiente, fuera del alcance de este cambio: si mas adelante se implementa la opcion (b) (rutear ambos frontends bajo el mismo dominio, por ejemplo con componentes de DigitalOcean App Platform), hay que actualizar `docs/CHECKLIST_OPERATIVO.md` (que hoy asume dos dominios separados para `CORS_ORIGINS`) y setear `VITE_ADMIN_PANEL_URL=/admin` en el build de produccion de `frontend/`.

Consecuencias: `backend/app/main.py` registra el `Limiter` (`app.state.limiter`), el manejador de `RateLimitExceeded` y `SlowAPIMiddleware`. `backend/app/api/v1/endpoints/habilitaciones.py` y `votos.py` decoran sus respectivos `POST` con `@limiter.limit(RATE_LIMIT_OPERATIVO)`, agregando `request: Request` a la firma (requisito de `slowapi` para poder identificar la request decorada). `backend/tests/conftest.py` gana un fixture `autouse` (`_reset_rate_limiter`) que llama `limiter.reset()` antes de cada prueba -- el `Limiter` es un singleton de modulo compartido por todo el proceso de `pytest`, y sin este reset los contadores se acumulan entre pruebas de archivos distintos y pueden producir un `429` inesperado en una prueba que no tiene nada que ver con rate limiting. `backend/tests/test_rate_limit.py` (nuevo, 3 pruebas): cada endpoint devuelve `429` al superar el limite configurado, y una prueba dedicada de `get_client_ip` que verifica que dos IPs distintas via `X-Forwarded-For` no comparten contador. `slowapi` se agrega a `requirements.txt`/`pyproject.toml` (no a `-dev`: corre en runtime, no solo en pruebas). 114 pruebas de backend en verde (111 previas + 3 nuevas).
